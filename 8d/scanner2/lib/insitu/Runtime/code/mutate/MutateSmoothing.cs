...
namespace insitu
{
	...
	public class MutateSmoothing : PoseBehaviour
	{
		...
		public void Update()
		{
			...
			if (Current.valid_position != 0)
				Current.position = double3.Smooth(Current.position, pose.position, ref PositionVelocity, deltaTime, Smoothing);

			if (Current.valid_rotation != 0)
			{
				Current.rotation = double4.Smooth(Current.rotation, pose.rotation, ref RotationVelocity, deltaTime, Smoothing);
			}
		}
