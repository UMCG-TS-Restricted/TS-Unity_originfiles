...
namespace insitu
{
	public static partial class Vicon
	{
		...
		public sealed class Worker : IDisposable
		{
			...
			public Worker(string host, int stream_mode, int vicon_mode)
			{
				var handle = GCHandle.Alloc(this, GCHandleType.Pinned);
			...
			///		It is advised to mark this as true.
			/// </param>
			public void Transform(double4x4 transform, bool delayed)
			{
				if (delayed)
				{
					QueueTransformArg = transform;
					QueueTransform = true;
				}
				else
				{
			...
			public static State InternalUpdate(Worker self, IntPtr dll, int vicon_mode)
			{
				...
				if (self.QueueTransform)
				{
					...
					var transform = self.QueueTransformArg;
